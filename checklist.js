/* ============================================================
   Fieldcheck — Shared Checklist Logic  v2.1
   ============================================================
...
function _applyStepState(code, val) {
  ...
  if (reasonEl) {
    if (val === 'flag') {
      ...
      // Inject photo UI if not already there
      _injectPhotoUI(code);
...
function generatePDF() {
  ...
  // Flagged summary — reasons + photos here only
  if (d.flagged.length > 0) {
    ...
    d.flagged.forEach(f => {
      ...
      if (photo) {
        ...
        // Scale to fit within usable page width at ~150dpi
        const mmPerPx = 25.4 / 150;
        let imgW = Math.min(nw * mmPerPx, cw - 4);
        let imgH = nh * (imgW / (nw * mmPerPx)) * mmPerPx;
        // Never let a photo take up more than 2/3 of a page height
